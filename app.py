...
def loginAction():
  ...
  if form.validate_on_submit(): # respond to form submission
      ...
      if user and user.check_password(data['password']): # check credentials
        flash('You have been logged in successfully!') # send message to next page
        login_user(user) # login the user
        return redirect(url_for('index')) # redirect to homepage if login successful
...
def signupAction():
  ...
  if form.validate_on_submit():
    ...
    db.session.commit()
    flash('Congratulations, your account has been successfully created!')# send message
...
def create_list():
  ...
  db.session.add(list)
  db.session.commit()
  flash('Item have been added to your list!')
...
def toggle(id):
  ...
  flash('Got item!') if list.done else flash ('Item still need to purchase or get!')
  db.session.add(list)
  db.session.commit()
  return redirect(url_for('get_list'))
...
def edit_list(id):
  ...
  if list:
    ...
    flash('Shopping list not found or unauthorized access!')
  return redirect(url_for('get_list'))
...
def delete_list(id):
  list = List.query.filter_by(userid=current_user.id, id=id).first()
  if list == None:
    flash ('Invalid id or unauthorized access!')
...
def edit_action(id):
  ...
  if list:
    ...
    flash('List not found or unauthorized access!')
  return redirect(url_for('get_list'))  
